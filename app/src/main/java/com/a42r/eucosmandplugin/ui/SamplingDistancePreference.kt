...
/**
 * Custom preference for minimum sampling distance with combined SeekBar and EditText.
 * Range: 0.5 to 200 km
 * - 0.5-10 km: 0.5 km increments
 * - 10-30 km: 1 km increments
 * - 30-100 km: 5 km increments
 * - 100-200 km: 10 km increments
 */
class SamplingDistancePreference @JvmOverloads constructor(
    context: Context,
    ...
    companion object {
        ...
        private const val MAX_VALUE = 200f
        
        // Calculate seekbar positions with variable increments
        private val positions: List<Float> = buildList {
            // 0.5-10 km: 0.5 km increments (19 steps)
            var value = MIN_VALUE
            while (value <= 10f) {
                add(value)
                value += 0.5f
            }
            // 10-30 km: 1 km increments (20 steps)
            value = 11f
            while (value <= 30f) {
                add(value)
                value += 1f
            }
            // 30-100 km: 5 km increments (14 steps)
            value = 35f
            while (value <= 100f) {
                add(value)
                value += 5f
            }
            // 100-200 km: 10 km increments (10 steps)
            value = 110f
